...
/// </summary>
public class AccountRepository                                              //Innehåller liknande kod som PostRepository.cs. Har kommenterat där.
{
    ...
    public static void Add(string username, string password, string email, IEnumerable<int> roles)
    {
        using(var db = Database.Open(_connectionString))
        {
            ...
            if(user != null)
            {
                throw new Exception("User already excist");
            }
    ...
    public static void AddRoles(int userId, IEnumerable<int> roles, Database db)
    {
        if (!roles.Any())
        {
            return;
